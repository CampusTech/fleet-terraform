...
locals {
  ...
  # LOCAL FIX: align with the `container_port > 0` sentinel used in the
  # dynamic ports block below. Using `length(ports)` here would pick a
  # sidecar with container_port=0 as the ingress container, since defaults
  # always fill the inner fields after type unification.
  ingress_container = try(
    [for container in var.containers : container if try(container.ports.container_port, 0) > 0][0],
    null
  )
  ...
  template {
    ...
    dynamic "containers" {
      ...
      content {
        ...
        # LOCAL FIX (vs. upstream GoogleCloudPlatform/terraform-google-cloud-run):
        # The upstream check `lookup(...) != {}` never resolves to false because
        # `optional()` fills the inner field defaults during type unification, so
        # `containers.value.ports` is always non-empty even for sidecars that
        # don't expose an ingress port. Cloud Run rejects services where more
        # than one container has a ports block. We treat `container_port = 0`
        # as a sentinel for "this container has no exposed port" — callers set
        # that on sidecars (e.g. Datadog DDOT agent) to suppress the block.
        dynamic "ports" {
          for_each = try(containers.value.ports.container_port, 0) > 0 ? [containers.value.ports] : []
          content {
